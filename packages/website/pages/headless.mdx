...
```jsx
useSubscription(model, () => {}, events, (deps = []));
```
...
```jsx
<Subscription model={model} events={events} getHash={getHash}>
    {model => {}}
</Subscription>
```
...
-   `events: Array<Integer>`
    -   **Required**
    -   Defaults to: <Link href="/headless#numeric-event-constants"><a>`EVT_ALL`</a></Link>
-   `getHash: Function() => String | Integer`
    -   **Required**
    -   model entity hash
