...
- 70b2b16: Replace array-based events with bitmask subscriptions and stable
  revisions, move range mapping into core, and add the framework-neutral DOM
  layout adapter. `VirtualScrollerLayout` now owns client-side virtual geometry
  styles and does not serialize them during SSR. Dynamic-size scrolling remains
  stable during native scrollbar drags and programmatic scrolling.

    Add coded `VirtualScrollerError` failures, atomic runtime validation, item-size
    invalidation, collection splice, and terminal disposal APIs. Improve
    measurement reuse, estimated-size cache resets, viewport anchoring, sticky
    positioning, and final-range publication after native scrollbar release.
    Browser and Node entrypoints are split so only Node installs the server-side
    ResizeObserver fallback. Remove the redundant `resetMeasurements` alias.
