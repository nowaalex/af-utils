...
- 70b2b16: Refactor the adapter around `useVirtualSnapshot`, `useVirtualEffect`,
  `useVirtualLayout`, and stable item-ref hooks. Adopt the framework-neutral DOM
  layout resource with component-lifetime identity, expose element refs instead of
  adapter-owned geometry, and stop serializing virtual geometry during SSR. Rename
  list item `i` to `index`.
