    ...
    */
    ...
    _itemCountChanged( itemCount ){
        ...
        if( itemCount < 0 || itemCount > 0x7fffffff ){
            ...
            */
            throw new Error( `itemCount must be 0 - 2_147_483_647. Got: ${itemCount}.` )
        }
    ...
    getIndex( offset ){
        ...
        for( let bitMask = this._msb, tempIndex = 0; bitMask !== 0; bitMask >>= 1 ){
    ...
    _measureItems(){
        ...
        */
        let child = this._zeroChildNode && this._zeroChildNode.nextElementSibling;
    ...
    _shiftRangeToEnd(){
        ...
        const from = this.getIndex( Math.max( 0, this.scrollSize - this._widgetSize ) );
    ...
    setHorizontal( horizontal ){
        if( horizontal !== this.horizontal ){
            ...
            this._scrollKey = horizontal ? HORIZONTAL_SCROLL_KEY : VERTICAL_SCROLL_KEY;
            this._sizeKey = horizontal ? HORIZONTAL_SIZE_KEY : VERTICAL_SIZE_KEY;
            if( this._outerNode ){
                /* TODO: Needs testing */
                this._scrollPos = 0;
                this.setWidgetSize( this._outerNode[ this._sizeKey ] );
