...
const submitIgnoringConstraints = (input: Locator) =>
    input.evaluate(element => {
        const form = (element as HTMLInputElement).form;
        if (!form) throw new Error("Expected input to belong to a form");
        form.dispatchEvent(
            new SubmitEvent("submit", { bubbles: true, cancelable: true })
        );
    });

await describeExample("virtual/scrolling/scroll-to-item", example => {
    test("documentation preview validates both forms without sandbox errors", async ({
        page
    }) => {
        ...
        page.on("console", message => {
            ...
            if (
                text.includes("Blocked form submission") ||
                (text.includes("sandboxed") && text.includes("allow-forms"))
            ) {
                blockedSubmissions.push(text);
            }
        });
        ...
        const iframeSelector = `iframe[src="${example.previewPath}"]`;
        ...
        expect(initialCount).toBe(50_000);

        const indexInput = preview.getByRole("spinbutton", {
        ...
        const rowsToAdd = 3;
        const nextCount = initialCount + rowsToAdd;
        const rowsInput = preview.getByRole("spinbutton", {
            name: "Rows to add:"
        });
        await rowsInput.fill(String(rowsToAdd));
        await preview
            .getByRole("button", { name: "Add and scroll to end" })
            .click();
        const finalItem = preview.locator(
            `[role="listitem"][aria-posinset="${nextCount}"][aria-setsize="${nextCount}"]`
        );
        await expect(finalItem).toBeAttached({ timeout: 10_000 });

        await indexInput.fill("-1");
        await submitIgnoringConstraints(indexInput);
        await indexInput.fill(String(nextCount));
        await submitIgnoringConstraints(indexInput);
        await indexInput.fill("1.5");
        await submitIgnoringConstraints(indexInput);
        await rowsInput.fill(String(-nextCount));
        await submitIgnoringConstraints(rowsInput);
        await rowsInput.fill("1000000");
        await submitIgnoringConstraints(rowsInput);
        await rowsInput.fill("1.5");
        await submitIgnoringConstraints(rowsInput);
