...
export async function generateStaticParams() {
    ...
    const result = glob.default
        ...
        .map(f => ({ example: f.split("/").slice(4, -2) }));
...
export async function generateMetadata({ params }: Params) {
    ...
    const descriptionModule = await import(
        `../../../../../examples/src/scrollend-polyfill/${params.example.join("/")}/meta.ts`
    );
...
const Page = async ({ params }: Params) => {
    ...
    const src = `/examples/src/scrollend-polyfill/${key}/index.html`;

    const { default: Description } = await import(
        `../../../../../examples/src/scrollend-polyfill/${key}/README.md`
    );

    const { default: codeString } = await import(
        `!!raw-loader!../../../../../examples/src/scrollend-polyfill/${key}/src/code.tsx`
    );
